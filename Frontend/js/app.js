...
const CATEGORIES = ["Personal", "Tech", "Travel", "Food", "Lifestyle", "Other"];
...
// TODO(Module 2/5): replace body with
//   const res = await fetch('/api/auth/profile', { method: 'PUT', headers: { Authorization: 'Bearer ' + token }, body: JSON.stringify({name}) })
function updateUserProfile({ name }) {
  ...
  // keep every post's byline in sync with the new display name
  if (name && name !== user.name) {
    const posts = readStore(DB_KEYS.POSTS, []);
    posts.forEach((p) => {
      if (p.author === user.name) p.author = name;
    });
  ...
  writeStore(DB_KEYS.USERS, users);
  writeStore(DB_KEYS.SESSION, { ...session, name: user.name });
  return user;
}
...
function getPostById(id) {
  return readStore(DB_KEYS.POSTS, []).find((p) => p.id === id) || null;
}
...
// TODO(Module 2): replace with `await fetch('/api/blogs/' + id, { method: 'PUT', body: ... })`
...
// TODO(Module 2): replace with `await fetch('/api/blogs?search=' + q + '&category=' + category)`
function searchPosts({ q = "", category = "All" } = {}) {
